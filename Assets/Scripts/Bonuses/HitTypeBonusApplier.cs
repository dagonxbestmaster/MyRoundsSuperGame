...
public class HitTypeBonusApplier : MonoBehaviour, BonusApplier
{
  ...
  public void ApplyBonus(List<BonusType> existingBonusTypes, GameObject root) // Применяем бонусы к пуле
  {
        ...
        int finalHitType = GetFinalHitType(existingBonusTypes);
        Apply(finalHitType, root);  // Применяем объект типа попадания к исходному объекту
  }
  private BulletHit GetHit(List<BonusType> existingBonusTypes) // Получаем объект типа попадания
    {
        int finalHitType = GetFinalHitType(existingBonusTypes);
        return CreateBulletHit(finalHitType); // Возвращаем объект типа попадания
    }

    private BulletHit CreateBulletHit(int finalHitType)
    {
    ...
    private static int GetFinalHitType(List<BonusType> existingBonusTypes)
    {
    ...
    private void Apply(int finalHitType, GameObject root) // Применяем объект типа попадания к исходному объекту
  {
        // Ищем все компоненты в дочерних объектах root
        // Которые зависят от типа попадания
        BulletHit hit = CreateBulletHit(finalHitType);
